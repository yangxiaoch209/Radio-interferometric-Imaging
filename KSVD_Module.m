function [y,D,nz] = KSVD_Module(params,msgdelta)
...
% gain %
% gain是一个与噪声强度相关的增益因子，通常用于调整去噪的灵敏度
...
ids = cell(p,1); % p代表信号的维度
if (p==1)
    ids{1} = reggrid(length(x)-blocksize+1, trainnum, 'eqdist'); % reggrid会为每个维度计算均匀分布的样本位置
else
    [ids{:}] = reggrid(size(x)-blocksize+1, trainnum, 'eqdist');
end
params.data = sampgrid(x,blocksize,ids{:}); % sampgrid使用之前生成的块索引ids和块大小blocksize从原始信号x中提取图像块
...
D = ksvd(params,verbose,msgdelta); % 字典学习
