...
temp_Dirtymap_1=temp_Dirtymap_1(51:150,51:150);
...
temp_Dirtymap_2=temp_Dirtymap_2(51:150,51:150);
...
temp_Dirtymap_3=temp_Dirtymap_3(51:150,51:150);
...
% 初始化
model_1=zeros(100,100);model_2=zeros(100,100);model_3=zeros(100,100);%空图像
times_1=0;times_2=0;times_3=0;%循环次数
BD_1=PSF;BD_2=PSF;BD_3=PSF;
flag_1=1;flag_2=1;flag_3=1;
gama= 0.5 ; %增益因子
times_break1=0;times_break2=0;times_break3=0;
sum=[sigma_1,sigma_2,sigma_3];
amax=max(sum);
Pmax1_min=inf;Pmax2_min=inf;Pmax3_min=inf;%初始值
...
%     model_1(tx,ty)=Pmax;%记录该峰值点源的幅度值以及位置，将其记录进空图像model中
    ...
    Pmax %更新后的最大值
    if    Pmax< 1 || times_1 > 350000 || times_break1>10  %达到阈值
        flag_1=0;
    end
end
...
%     model_2(tx,ty)=Pmax;%记录该峰值点源的幅度值以及位置，将其记录进空图像model中
    ...
    Pmax %输出最大值
    if    Pmax< 1 ||times_2>350000 || times_break2>10  %达到阈值
        flag_2=0;
    end
end
...
%     model_3(tx,ty)=Pmax;%记录该峰值点源的幅度值以及位置，将其记录进空图像model中
    ...
    Pmax %输出最大值
    if    Pmax< 1 ||times_3>350000  || times_break3>10 %达到阈值
        flag_3=0;
    end
end
...
% I=I_1+I_2+I_3+temp_Dirtymap_1+temp_Dirtymap_2+temp_Dirtymap_3;
I=I_1+I_2+I_3;

figure;pcolor(Original);shading interp;colorbar; title('原图');%caxis([0 10]);
figure;pcolor(Dirtymap);shading interp;colorbar; title('脏图');%caxis([0 10]);
figure;pcolor(model_1);shading interp;colorbar;title('模型图'); %caxis([0 10]);
figure;pcolor(model_2);shading interp;colorbar;title('模型图'); %caxis([0 10]);
figure;pcolor(model_3);shading interp;colorbar;title('模型图'); %caxis([0 10]);
figure;pcolor(temp_Dirtymap_1);shading interp;colorbar;title('残图1'); %caxis([0 10]);
figure;pcolor(temp_Dirtymap_2);shading interp;colorbar;title('残图2'); %caxis([0 10]);
figure;pcolor(temp_Dirtymap_3);shading interp;colorbar;title('残图3'); %caxis([0 10]);
