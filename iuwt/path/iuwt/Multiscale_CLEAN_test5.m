...
% 初始化
model_1=zeros(500,700);model_2=zeros(500,700);model_3=zeros(500,700);%空图像
...
while flag_1
    ...
    BD_1=circshift(BD_1,ty-ty_BD,2);%列移位
        ...
        % 尺度大小d和脏束BD进行卷积   进行卷积的BD每次不一样，无法提出来加速
    ksigma_temp=ksigma_1(101:600,1:700);
...
%     temp_Dirtymap_1=temp_Dirtymap_1.*S1;
    ...
    Pmax %更新后的最大值
    if    Pmax< 10 || times_1 > 350000 || times_break1>10  %达到阈值
        flag_1=0;
    end
end
...
while flag_2
    ...
    BD_2=circshift(BD_2,ty-ty_BD,2);%列移位
            ...
            % 尺度大小d和脏束BD进行卷积   进行卷积的BD每次不一样，无法提出来加速
    ksigma_temp=ksigma_2(101:600,1:700);
...
while flag_3
    ...
    BD_3=circshift(BD_3,ty-ty_BD,2);%列移位
            ...
            % 尺度大小d和脏束BD进行卷积   进行卷积的BD每次不一样，无法提出来加速
    ksigma_temp=ksigma_3(101:600,1:700);
...
b=b.*win;
I_1=conv2(model_1,b);
I_1=I_1(251:750,351:1050);
I_2=conv2(model_2,b);
I_2=I_2(251:750,351:1050);
I_3=conv2(model_3,b);
I_3=I_3(251:750,351:1050);
