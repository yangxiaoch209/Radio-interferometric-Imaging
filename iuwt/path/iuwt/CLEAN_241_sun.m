...
%% 载入原图、脏图、脏束
%导入jupyter T2副本生成的脏束
addpath('E:\Desktop\yx\射电天文\网格化与加权\Imaging-Tutorial-master');
A = importdata('T2-Bimage-1.csv');  %读取csv文件  脏束
PSF=A;
% 点扩散函数横截面的波形
[len1,~]=size(PSF);
...
% figure;pcolor(xi,eta,real(Original));shading interp;colorbar; title('原图');
figure;pcolor(xi,eta,Dirtymap);shading interp;colorbar;title('脏图'); 
figure;pcolor(xi,eta,real(model));shading interp;colorbar;title('模型图'); 
figure;pcolor(xi,eta,real(I));shading interp;colorbar;title('结果图'); 
