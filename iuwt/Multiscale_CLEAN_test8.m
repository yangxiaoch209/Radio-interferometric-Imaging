%% Multi-scale CLEAN  多尺度CLEAN      PSF取101*101，原图取101*101    test4修改而来的
clear;
clc;
close all;
...
model=zeros(Tc,Tc);model_1=zeros(Tc,Tc);model_2=zeros(Tc,Tc);model_3=zeros(Tc,Tc);%空图像
times=0;%循环次数
BD=PSF;
flag=1;
gama= 0.3 ; %增益因子
S=1; % 初始权重为1
yuzhi= 0.1*max(max(Dirtymap)) ; %迭代截止阈值
...
%     temp_Dirtymap_3=temp_Dirtymap_3.*S;
    ...
    times=times+1;  %循环次数
    Pmax %输出最大值
    if   Pmax < yuzhi || times> 350000  %达到阈值
        flag=0;
    end
end
toc % 计时
...
I_1=conv2(model,ksigma_1);
I_1=I_1(51:151,51:151);
I_1=conv2(I_1,b);
I_1=I_1(51:151,51:151);

I_2=conv2(model,ksigma_2);
I_2=I_2(51:151,51:151);
I_2=conv2(I_2,b);
I_2=I_2(51:151,51:151);

I_3=conv2(model,ksigma_3);
