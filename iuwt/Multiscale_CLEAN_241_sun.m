...
A = importdata('T2-Bimage-0.1.csv');  %读取csv文件  脏束
PSF=A;
...
figure;pcolor(xi,eta,Original);shading interp;colorbar; title('原图');caxis([0 200]);
figure;pcolor(xi,eta,PSF);shading interp;colorbar; title('PSF');
...
figure;pcolor(xi,eta,real(I));shading interp;colorbar;title('多尺度clean'); %caxis([0 200]);
